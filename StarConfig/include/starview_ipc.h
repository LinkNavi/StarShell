 ...
 * Enhanced with window management
 */
...
static bool starview_ipc_focus_window_by_title(StarViewIPC *ipc, const char *title) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "[title=\"%s\"] focus", title);
    return starview_ipc_command(ipc, cmd);
}

static bool starview_ipc_focus_window_by_app_id(StarViewIPC *ipc, const char *app_id) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "[app_id=\"%s\"] focus", app_id);
    return starview_ipc_command(ipc, cmd);
}
